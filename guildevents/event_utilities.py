        ...
        if signups and len(signups) > 0:
            ...
            for player_role, player_role_data in PLAYER_ROLES_DATA.items():
                ...
                if player_role in signups_by_role:
                    ...
                    player_names_joined = "\n".join([
                        f'\t {signup.player_mention} {"(Can Flex: " + signup.flex_roles + ")" if signup.flex_roles else ""} {signup.date_created.strftime("- %Y-%m-%d %H:%M") if extra == "extra" else ""}'
                        for signup in signups])
...
def convert_to_proper_timezone(event_time, new_channel):
    time = None
    if new_channel.guild.id != config.INCURABLE_SERVER_ID:
